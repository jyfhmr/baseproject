...
export class CreditNoteService {
    constructor(
        ...
        @InjectRepository(cyp_accounts_payable_documents_credit_note)
        private creditNoteRepository: Repository<cyp_accounts_payable_documents_credit_note>,

        @InjectRepository(config_admistrative_reason)
        private reasonRepository: Repository<config_admistrative_reason>,
    ) {}

    async create(createCreditNoteDto: CreateCreditNoteDto, userId: number) {
        const user = await this.usersService.findOne(userId);
        const { motive, ...otherFields } = createCreditNoteDto;
        const correlative = await this.correlativeService.generateCode('AXP', 'CDN');
        // Busca las entidades config_admistrative_reason por sus IDs
        const reasons = await this.reasonRepository.findByIds(motive);

        // Crea la entidad CreditNote con los otros campos y asigna los motivos y usuario
        const newMcreditNote = this.creditNoteRepository.create({
            ...otherFields,
            user, // Asigna el usuario
            motive: reasons, // Asigna el array de motivos
            correlative, // Asigna el array de motivos
        });

        // Guarda la nueva entidad en la base de datos
        await this.creditNoteRepository.save(newMcreditNote);
    ...
    async findAll(
        query: any,
    ): Promise<{ totalRows: number; data: cyp_accounts_payable_documents_credit_note[] }> {
        ...
        try {
            const [resCount, resData] = await Promise.all([
                this.creditNoteRepository.count({ where }),
                query?.export
    ...
    async findOne(id: number): Promise<cyp_accounts_payable_documents_credit_note> {
        const relations = {
            user: true,
            userUpdate: true,
            typeMoney: true,
        ...
        return await this.creditNoteRepository.findOne({ where: { id: id }, relations });
    }

    async update(id: number, updateCreditNoteDto: UpdateCreditNoteDto, userId: number) {
        // Busca el usuario que realizó la actualización
        const user = await this.usersService.findOne(userId);
        console.log(updateCreditNoteDto);
        ...
        const { motive, ...otherFields } = updateCreditNoteDto;

        let reasons = [];
        if (motive && motive.length > 0) {
            // Busca las entidades config_admistrative_reason por sus IDs
            reasons = await this.reasonRepository.findByIds(motive);
        }

        // Actualiza los campos de la nota de crédito, incluyendo los motivos si es necesario
        creditNote.motive = reasons.length > 0 ? reasons : creditNote.motive; // Mantén los motivos existentes si no se actualizan
        Object.assign(creditNote, otherFields); // Actualiza otros campos con los datos de updateCreditNoteDto
    ...
    async changeStatus(id: number): Promise<string | Error> {
        const updateStatus = await this.creditNoteRepository.findOneBy({ id });
        updateStatus.isActive = !updateStatus.isActive;

        try {
            await this.creditNoteRepository.save(updateStatus);
    ...
    async processExcel(buffer: Buffer): Promise<any> {
        ...
        await this.creditNoteRepository.save(entities);

        return { message: 'Datos importados correctamente', total: entities.length };
    }

    async listMoney(): Promise<cyp_accounts_payable_documents_credit_note[]> {
        return await this.creditNoteRepository.find({ where: { isActive: true } });
    }
