...
export class AccountService {
    ...
    async create(
        ...
        const { commissions, ...accountDetails } = createAccountDto; // Separa las comisiones del resto de los detalles

        // Crear la nueva cuenta
        const newAccount = this.accountRepository.create(accountDetails);
        newAccount.user = user;
        newAccount.bank = bank;
        // Guarda la nueva cuenta
        const savedAccount = await this.accountRepository.save(newAccount);

        // Crear comisiones asociadas
        if (commissions && commissions.length > 0) {
            ...
            await this.comissionRepository.save(comissionEntities); // Guardar todas las comisiones
        }

        // Refresca el banco y sus cuentas para incluir la nueva
        const updatedBank = await this.banksService.findOne(bankId, ['accounts']); // Incluye las cuentas
        console.log('Updated Bank Accounts:', updatedBank.accounts);
    ...
    async findOne(id: number): Promise<Treasury_maintenance_Account> {
        return await this.accountRepository.findOne({
            ...
            relations: [
                'bank',
                'commissions',
                'commissions.paymentMethod', // Añadir esta línea
            ],
        });
    }
