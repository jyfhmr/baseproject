...
export class Rates2RangesService {
  ...
  private buildWhereClause(query: any) {
    ...
    if (query.minimumAmountPaid) {
      where.minimumAmountPaid = Raw((alias) => `${alias} LIKE '%${query.minimumAmountPaid}%'`);
    }

    // Filtro por maximo
    if (query.maximumAmountPaid) {
      where.maximumAmountPaid = Raw((alias) => `${alias} LIKE '%${query.maximumAmountPaid}%'`);
    }

    // Filtro por maximo
    if (query.retentionPorcentage) {
      where.retentionPorcentage = Raw((alias) => `${alias} LIKE '%${query.retentionPorcentage}%'`);
    }
