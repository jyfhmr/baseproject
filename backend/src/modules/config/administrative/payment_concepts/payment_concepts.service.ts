...
export class PaymentConceptsService {
    ...
    async listPaymentConcept(): Promise<config_admistrative_paymentConcept[]> {
        // return await this.paymentConceptRepository.find({ where: { isActive: true } });
        return await this.paymentConceptRepository
            .createQueryBuilder('config_administrative_payment_concepts')
            .select([
                "CONCAT(config_administrative_payment_concepts.name, ' - ', config_administrative_payment_concepts.numeroLiteral) as concept",
                'config_administrative_payment_concepts.id as id',
            ])
            .where('config_administrative_payment_concepts.isActive = :isActive', {
                isActive: true,
            })
            .getRawMany();
    }
