...
const moduleName = 'config/actions';
...
const ModuleForm = ({ params }: any) => {
    ...
    const pageName = `${id ? 'Editar' : 'Nueva'} Acción`;
    const iconPage = <AppstoreOutlined />;
    const breadCrumb = [{ title: 'Configuración' }, { title: 'Acciones' }, , { title: pageName }];
    ...
    const onFinish: FormProps<FieldType>['onFinish'] = async (values) => {
        try {
            ...
            toast.success(`Acción ${id ? 'editada' : 'creada'} con éxito`);
        } catch (error) {
            setLoader(false);
