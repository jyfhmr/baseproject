...
const TableProducts = ({
    ...
    const fetchData = async () => {
        ...
        try {
            const productsId = products.map((el: any) => el.id);
            setData([]);
            ...
            const params = {
                ...
                order: order,
                search: searchText,
                productsId, // Otros parámetros de búsqueda personalizados
            };

            const getList = await getData('inventory/catalogue', params);
    ...
    const columns: any = [
        ...
        { title: 'Código de barra', dataIndex: 'barcode', width: 150, fixed: 'left' },
        { title: 'Producto', dataIndex: 'name', width: 200, fixed: 'left' },
        {
            title: 'Categoría',
            dataIndex: ['subCategory', 'category', 'name'],
            width: 100,

            render: (text: string, record: DataTypeActions) =>
