...
import { EditOutlined, RetweetOutlined, DeleteOutlined, EyeOutlined, FilePdfFilled } from '@ant-design/icons';
...
import '@/components/dashboard/style/table.css';
...
const TableProviders = ({ columnsProp, dataSource, moduleName, totalRows, expandable }: any) => {
    ...
    // Estado para controlar la visibilidad del modal y los datos del proveedor
                ...
                <>
                    {item.actions.map((action: any, index: number) => (
                        <span key={`${record.id}-${index}`}>
                            ...
                            {action.type === 'edit' && actions.includes(3) && (
                            ...
                            {action.type === 'download' && actions.includes(3) && (
